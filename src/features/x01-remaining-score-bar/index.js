...
function buildDebugMessage(debugInfo = {}) {
  ...
  return `${baseMessage} visuals(colors="${visuals.colorTheme || "checkout-focus"}", size="${
    visuals.barSize || "standard"
  }", effect="${visuals.effect || "bar-pulse"}")`;
}
...
export function mountX01RemainingScoreBar(context = {}) {
  ...
  const featureConfig =
    config && typeof config.getFeatureConfig === "function"
      ? config.getFeatureConfig("x01RemainingScoreBar")
      : {
...
export const initializeX01RemainingScoreBar = mountX01RemainingScoreBar;
export const initialize = mountX01RemainingScoreBar;
export const mount = mountX01RemainingScoreBar;
