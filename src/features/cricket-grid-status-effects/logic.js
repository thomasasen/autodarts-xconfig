...
export function createCricketGridStatusEffectsState(windowRef = null) {
  return {
    windowRef,
    gridRoot: null,
...
function setCricketGridStatusEffectsFailure(state, debugStats, status) {
...
export function updateCricketGridStatusEffects(options = {}) {
  ...
  initializeCricketGridStatusEffectsDebugStats(debugStats);

  if (!documentRef || !cricketRules || !renderState || !state || !visualConfig) {
    setCricketGridStatusEffectsFailure(state, debugStats, "invalid-input");
    return;
  }
  ...
  if (!targetOrder.length || !(renderState.stateMap instanceof Map)) {
    setCricketGridStatusEffectsFailure(state, debugStats, "invalid-state");
    return;
  }
  ...
  if (!gridRoot) {
    setCricketGridStatusEffectsFailure(state, debugStats, "missing-grid");
    return;
  }
