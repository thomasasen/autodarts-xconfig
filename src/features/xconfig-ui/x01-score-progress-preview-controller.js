...
const PREVIEW_BAR_SELECTOR = "[data-adxconfig-x01-score-progress-preview-loop]";
const PREVIEW_SCORE_SELECTOR = "[data-adxconfig-x01-score-progress-preview-score='true']";
const PREVIEW_ROUTE_SELECTOR = "[data-adxconfig-x01-score-progress-preview-route='true']";
...
const PREVIEW_LOOPS = Object.freeze({
  ...
  "ghost-trail-drop": GHOST_TRAIL_PREVIEW_STEPS,
});
...
function readPreviewConfig(hostNode) {
  return {
    ...
    effect: hostNode?.getAttribute?.(EFFECT_ATTRIBUTE) || "pulse-core",
  };
}
...
function getPreviewLoopName(hostNode) {
  const loopName = String(
    hostNode?.getAttribute?.("data-adxconfig-x01-score-progress-preview-loop") || ""
  ).trim();
...
export function createX01ScoreProgressPreviewController(options = {}) {
