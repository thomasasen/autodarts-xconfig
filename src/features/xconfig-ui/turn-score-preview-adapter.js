import {
  animateScore as animateTurnScoreScore,
  stopAnimation as stopTurnScoreAnimation,
} from "../turn-score-counter/logic.js";
...
import {
  ...
  TURN_SCORE_PREVIEW_SCORE_SELECTOR,
...
function normalizeCountEffect(value) {
  ...
  if (normalized === "rolling-digits" || normalized === "step-count") {
    return normalized;
  }
  if (normalized === "odometer" || normalized === "rolling") {
    return "rolling-digits";
  }
  if (normalized === "steps" || normalized === "step" || normalized === "integer") {
    return "step-count";
  }
  return "smooth-count";
}
...
function resolvePreviewOptions(context) {
  ...
  if (settingKey === "countEffect" && settingValue) {
    ...
    options.countEffect = "smooth-count";
...
function createIdleScoreNode(documentRef) {
  ...
  scoreNode.setAttribute(TURN_SCORE_PREVIEW_SCORE_ATTRIBUTE, "true");
  return scoreNode;
}
...
function resetPreviewScoreNode(optionNode, state, windowRef, stopAnimationRef, scoreNode = null) {
  const currentScoreNode =
    scoreNode || optionNode?.querySelector?.(TURN_SCORE_PREVIEW_SCORE_SELECTOR) || null;
...
export function createTurnScoreCounterPreviewAdapter(options = {}) {
  ...
  const animateScoreRef =
    typeof options.animateScore === "function" ? options.animateScore : animateTurnScoreScore;
  const stopAnimationRef =
    typeof options.stopAnimation === "function" ? options.stopAnimation : stopTurnScoreAnimation;
  ...
  function start(context = {}) {
    ...
    function run(odometerPluginRef = null) {
      ...
      if (previewOptions.countEffect === "rolling-digits" && odometerPluginRef) {
    ...
    if (previewOptions.countEffect === "rolling-digits") {
  ...
  return {
    prefix: "turn-score-counter-",
    matches: (previewEffect) => String(previewEffect || "").startsWith("turn-score-counter-"),
    start,
  };
}
