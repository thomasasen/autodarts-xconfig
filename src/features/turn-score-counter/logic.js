...
const COUNT_EFFECT_STEP = "step-count";
const COUNT_EFFECT_ALIASES = Object.freeze({
  countup: COUNT_EFFECT_SMOOTH,
  smooth: COUNT_EFFECT_SMOOTH,
  odometer: COUNT_EFFECT_ROLLING,
  rolling: COUNT_EFFECT_ROLLING,
  steps: COUNT_EFFECT_STEP,
  step: COUNT_EFFECT_STEP,
  integer: COUNT_EFFECT_STEP,
});
...
function normalizeCountEffect(value) {
  const normalized = String(value || "")
    .trim()
    .toLowerCase();
  if (normalized === COUNT_EFFECT_ROLLING || normalized === COUNT_EFFECT_STEP) {
    return normalized;
  }
  if (COUNT_EFFECT_ALIASES[normalized]) {
    return COUNT_EFFECT_ALIASES[normalized];
  }
  return COUNT_EFFECT_SMOOTH;
}
...
function startCountUpAnimation(node, state, options = {}) {
  ...
  const OdometerRef =
    countEffect === COUNT_EFFECT_ROLLING && typeof options.odometerPluginRef === "function"
...
export function updateTurnScore(options = {}) {
