...
export async function playSingleBullHitSoundPreview(options = {}) {
  const windowRef =
    options.windowRef ||
  ...
  if (!audio) {
    throw new Error("Single Bull Hit Sound audio is not available.");
  }
  ...
  try {
    ...
    throw new Error("Single Bull Hit Sound preview playback failed.", { cause: error });
  }
...
export function createSingleBullHitSoundState(windowRef, config) {
  return {
    windowRef,
...
export function clearSingleBullHitSoundState(state) {
  if (!state) {
    return;
  }
...
export function installSingleBullHitSoundPolling(state, callback, pollIntervalMs) {
