...
export function initializeDartMarkerReplacer(context = {}) {
  ...
  const featureConfig =
    config && typeof config.getFeatureConfig === "function"
      ? config.getFeatureConfig("dartMarkerReplacer")
        : {
            design: "autodarts",
            animateDarts: true,
            sizePercent: 120,
  ...
  const visualConfig = resolveDartMarkerReplacerConfig(featureConfig);
...
export async function runDartMarkerReplacerAction(actionContext = {}) {
  const actionId = String(actionContext.actionId || "").trim().toLowerCase();
  if (actionId !== "preview") {
    throw new Error(`Unsupported Dart Marker Replacer action: ${actionId || "unknown"}`);
  }

  return runDartMarkerReplacerPreview({
...
export const mountDartMarkerReplacer = initializeDartMarkerReplacer;
export const initialize = initializeDartMarkerReplacer;
export const mount = initializeDartMarkerReplacer;
