...
| --- | --- | --- | --- |
| `Animation/Autodarts Animate AVG Trend Arrow.user.js` | Trendpfeil am AVG | keine eigene Regelwahrheit | hoch |
| `Animation/Autodarts Animate Checkout Target Highlights.user.js` | markiert Checkout-Ziele am Board | X01-Checkout-Ziele, Bull-Mapping | hoch |
| `Animation/Autodarts Animate Checkout Score Highlight.user.js` | hebt checkoutfähige Restwerte hervor | X01-Checkout-/Out-Mode-Relevanz | hoch |
| `Animation/Autodarts Animate Cricket Grid Status Effects.user.js` | animiert Cricket-/Tactics-Matrix | Cricket-Zielzustände und Übergänge | hoch |
...
| `Animation/Autodarts Animate Dart Marker Replacer.user.js` | ersetzt Marker durch Dart-Bilder | keine eigene Regelwahrheit | mittel |
| `Animation/Autodarts Animate Dartboard Marker Highlight.user.js` | verstärkt Board-Marker | keine eigene Regelwahrheit | mittel |
| `Animation/Autodarts Animate Take Out Darts Alert.user.js` | betont Entfernen-der-Darts-Hinweis | keine eigene Regelwahrheit | mittel |
| `Animation/Autodarts Animate Single Bull Hit Sound.user.js` | spielt Sound bei Single Bull | Bull-Erkennung | mittel |
| `Animation/Autodarts Animate Special Hit Highlights.user.js` | hebt Triple-, Double- und Bull-Treffer hervor | Trefferklassifikation | hoch |
| `Animation/Autodarts Animate Turn Score Counter.user.js` | zählt Punkteänderungen sichtbar | Game-State-Diff | mittel |
| `Animation/Autodarts Animate Active Player Sweep.user.js` | markiert Spielerwechsel | keine eigene Regelwahrheit | mittel |
| `Animation/Autodarts Animate TV Board Zoom.user.js` | zoomt auf relevante Board-Bereiche | X01-Bust-/Checkout-Regeln | hoch |
| `Animation/Autodarts Animate Winner Celebration Effect.user.js` | Sieger-Overlay mit Test-Button | Siegererkennung, Bull-off-Ausnahme | hoch |
| `Animation/Autodarts Checkout Suggestion Styles.user.js` | formatiert Checkout-Empfehlungen | X01-Suggestion-Parsing | hoch |
...
| `assets/Dart_*.png` | Dart-Designs für `Dart Marker Replacer` | keine | mittel |
| `assets/TakeOut.png` | Grafik für `Take Out Darts Alert` | keine | mittel |
| `assets/singlebull.mp3` | Audio-Asset für `Single Bull Hit Sound` | Bull-Feedback | mittel |
| `assets/AD-xConfig.png` | Screenshot der Legacy-Konfigurationsoberfläche | keine | hoch |
| `assets/xConfig-testbutton.png` | Screenshot des Winner-Celebration-Effect-Testbuttons | keine | hoch |
