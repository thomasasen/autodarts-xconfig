...
test("cricket theme keeps standard preview placement and uses stable board layout hooks", () => {
  ...
  assert.match(css, /--ad-ext-theme-cricket-player-gap:\s*0rem;/);
  ...
  assert.match(
    css,
    /#ad-ext-player-display\s+\.ad-ext-player\s+\.ad-ext-player-score\s*\{[^}]*color:\s*var\(--ad-ext-theme-cricket-score-active-color\);[^}]*font-size:\s*clamp\(2\.85rem,\s*3\.45vw,\s*3\.7rem\)\s*!important;[^}]*line-height:\s*0\.9\s*!important;[^}]*font-weight:\s*800\s*!important;[^}]*text-align:\s*right\s*!important;[^}]*text-shadow:\s*var\(--ad-ext-theme-cricket-score-shadow\)\s*!important;/s
  );
  assert.match(
    css,
    /#ad-ext-player-display\s+\.ad-ext-player\.ad-ext-player-active\s+\.ad-ext-player-score\s*\{[^}]*font-size:\s*clamp\(3\.16rem,\s*3\.85vw,\s*4\.05rem\)\s*!important;/s
  );
  assert.match(
    css,
    /#ad-ext-player-display\s+\.ad-ext-player\.ad-ext-player-inactive\s+\.ad-ext-player-score,\s*#ad-ext-player-display\s+\.ad-ext-player:not\(\.ad-ext-player-active\):not\(\.ad-ext-player-winner\)\s+\.ad-ext-player-score\s*\{[^}]*font-size:\s*clamp\(2\.28rem,\s*2\.95vw,\s*3\.35rem\)\s*!important;[^}]*color:\s*var\(--ad-ext-theme-cricket-score-inactive-color\)\s*!important;/s
  );
  assert.match(
    css,
  ...
  assert.match(
    css,
    /#ad-ext-player-display\s+\.ad-ext-player\s*>\s*\.chakra-stack\.css-y3hfdd\s*>\s*\.chakra-stack\.css-1igwmid\s*\{[^}]*grid-area:\s*stats\s*!important;[^}]*grid-column:\s*1\s*\/\s*3\s*!important;[^}]*justify-self:\s*stretch\s*!important;[^}]*min-width:\s*var\(--ad-ext-theme-cricket-stats-min-width\)\s*!important;[^}]*display:\s*flex\s*!important;[^}]*justify-content:\s*flex-start\s*!important;[^}]*gap:\s*0\.2rem\s*!important;[^}]*padding-left:\s*1\.38rem\s*!important;/s
  );
  assert.match(
    css,
  ...
  assert.match(
    css,
    /#ad-ext-player-display\s+\.ad-ext-player\s*>\s*\.chakra-stack\.css-y3hfdd\s*>\s*\.chakra-stack\.css-37hv00\s*>\s*\.css-aa7b80\s*>\s*span\s*\{[^}]*display:\s*grid\s*!important;[^}]*grid-template-columns:\s*var\(--ad-ext-theme-cricket-player-avatar-size\)\s*minmax\(0,\s*1fr\)\s*auto\s*!important;[^}]*grid-template-areas:\s*"avatar name wins"\s*!important;[^}]*border:\s*none\s*!important;[^}]*padding:\s*0\.15rem 0 0\.2rem\s*!important;/s
  );
  assert.match(
    css,
    /#ad-ext-player-display\s+\.ad-ext-player\s*>\s*\.chakra-stack\.css-y3hfdd\s*>\s*\.chakra-stack\.css-37hv00\s*>\s*\.css-aa7b80\s*\{[^}]*margin-inline:\s*0\s*!important;[^}]*width:\s*100%\s*!important;/s
  );
  assert.match(
    css,
  ...
  assert.match(
    css,
    /#ad-ext-player-display\s+\.ad-ext-player\s*>\s*\.chakra-stack\s*>\s*\.ad-ext-player-score\s*\{[^}]*grid-area:\s*score\s*!important;[^}]*grid-column:\s*3\s*!important;[^}]*grid-row:\s*2\s*!important;[^}]*justify-self:\s*end\s*!important;[^}]*align-self:\s*center\s*!important;[^}]*margin-right:\s*0\s*!important;[^}]*padding-left:\s*0\.2rem\s*!important;[^}]*padding-right:\s*0\.18rem\s*!important;[^}]*max-inline-size:\s*none\s*!important;[^}]*overflow:\s*visible\s*!important;[^}]*z-index:\s*3\s*!important;/s
  );
  assert.match(
    css,
    /#ad-ext-player-display\s+\.ad-ext-player\.ad-ext-player-active\s+\.ad-ext-player-name,\s*#ad-ext-player-display\s+\.ad-ext-player\.ad-ext-player-active\s+\.ad-ext-player-name\s*>\s*p\s*\{[^}]*color:\s*var\(--ad-ext-theme-cricket-score-active-color\)\s*!important;/s
  );
  assert.doesNotMatch(
    css,
  ...
  assert.match(
    css,
    /#grid\s+tr\s*>\s*td:not\(:first-child\),\s*#grid\s+tr\s*>\s*th:not\(:first-child\)\s*\{[^}]*min-width:\s*var\(--ad-ext-theme-cricket-player-column-min-width\)\s*!important;[^}]*max-width:\s*var\(--ad-ext-theme-cricket-player-column-max-width\)\s*!important;[^}]*width:\s*var\(--ad-ext-theme-cricket-player-column-width\)\s*!important;/s
  );
  assert.match(
    css,
  ...
  assert.match(
    css,
    /\.ad-ext-theme-content-slot\s*\{[^}]*grid-template-columns:\s*minmax\(var\(--ad-ext-theme-cricket-player-area-required-width\),\s*max-content\)\s*minmax\(var\(--ad-ext-theme-cricket-board-min-width-auto\),\s*1fr\)\s*!important;/s
  );
  assert.match(
    css,
    /\.ad-ext-theme-content-slot\s*>\s*\.ad-ext-theme-content-left\s*\{[^}]*min-width:\s*0\s*!important;[^}]*min-height:\s*0\s*!important;[^}]*width:\s*max-content\s*!important;[^}]*height:\s*100%\s*!important;[^}]*max-width:\s*100%\s*!important;[^}]*justify-self:\s*start\s*!important;[^}]*align-self:\s*stretch\s*!important;/s
  );
  assert.match(
    css,
...
test("cricket theme strengthens tactical hierarchy without leaving stable hooks", () => {
  ...
  assert.match(
    css,
    /#ad-ext-player-display\s+\.ad-ext-player\s*>\s*\.chakra-stack\s*>\s*\.ad-ext-player-score\s*\{[^}]*max-inline-size:\s*none\s*!important;[^}]*overflow:\s*visible\s*!important;/s
  );
  assert.match(
    css,
