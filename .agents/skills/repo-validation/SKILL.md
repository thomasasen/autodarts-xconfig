...
- when SonarQube is reachable and the change affects Sonar-scanned JS/MJS source or project-level analysis settings, consider a SonarQube scan as an additional validation layer; use it to complement repo tests and lint, not to replace them
...
- use SonarQube validation when the change affects `src/**`, `loader/**`, `scripts/**`, `tests/**`, `sonar-project.properties`, or Sonar-specific repo guidance and the server is available; skip it for docs-only edits unless the task itself is about SonarQube or analysis policy
